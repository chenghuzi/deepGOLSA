...
from models import GoalReducer, VAEGoalReducer
from state_graphs import (
    SamplingStrategy,
    BaseStageGraph,
    FourRoomSG,
    RandomSG,
...
from utils import confusion_index, make_res_dir
...
def train(ctx, gr: str, lr: float, cores: int, graph: str, walk_repeats: int, scalfactor: float, klw: float, lm: Optional[int] = None):
    ...
    for state_graph in state_graphs:
        ...
        for sampling_strategy in ctx.obj["sampling_strategies"]:
            ...
            for _ in pbar:
                ...
                for _ in range(run_repeats):
                    ...
                    loss = loss.mean()
                    loss.backward()
                    torch.nn.utils.clip_grad_norm_(goal_reducer.parameters(), max_norm=1)
                    opt.step()
                    ...
                    optimality_list.append(optimality)
                    sg_equidex_list.append(sg_equidex)
        ...
        torch.save(
            ...
            dat_dir / f"{state_graph}{fignextra}-training.pt",
        )
...
def analyze_optimality(
    ...
    for sampling_strategy in sampling_strategies:
        ...
        path = np.array(
            [
                all_sg_pairs[to_viz_idx][0],  
                *all_s_g_indices_preds[to_viz_idx].tolist(),  
                all_sg_pairs[to_viz_idx][1],  
            ]
        )
