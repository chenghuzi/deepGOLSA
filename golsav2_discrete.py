...
from models import GoalReducer, InVisOutDiscAgentNet, VAEGoalReducer  
...
class GOLSAv2DQL(PolicyBase):
    ...
    def learn_subgoal(self, batch):
        ...
        with torch.no_grad():
            ...
            final_reached_goal_img_inputs = np.swapaxes(batch.final_reached_goal['image'], -3, -1)  
    ...
    def align_value_func(self, batch, epsilon=1e-16):
        ...
        with torch.no_grad():
            ...
            goal_img_inputs = np.swapaxes(batch.obs['desired_goal']['image'], -3, -1)  
