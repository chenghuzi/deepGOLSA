...
from c_utils import gen_ind_res  
from golsa_base import PolicyBase
from neural_nets import ImgObsAgentNet
...
class GOLSAv2DQL(PolicyBase):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.extract_batch_goal = extract_batch_goal  
    ...
    def learn_subgoal(self, batch, eps=1e-16):
        """Train a goal reducer."""
        model: ImgObsAgentNet = getattr(self, 'model')

        with torch.no_grad():
            
            target_subgoal_encoding = model.encode_g(self.extract_batch_goal(batch.subgoal)).data

        state_encoding = model.encode_s(batch.obs['observation'])
        goal_encoding = model.encode_g(self.extract_batch_goal(batch.final_reached_goal))
    ...
    def align_value_func(self, batch, epsilon=1e-16):
        model: ImgObsAgentNet = getattr(self, 'model')
        model_old: ImgObsAgentNet = getattr(self, 'model_old')
        with torch.no_grad():
            state_encoding_old = model_old.encode_s(batch.obs['observation'])
            goal_encoding_old = model_old.encode_g(self.extract_batch_goal(batch.obs['desired_goal']))
        ...
        state_encoding = model.encode_s(batch.obs['observation'])
        goal_encoding = model.encode_g(self.extract_batch_goal(batch.obs['desired_goal']))
...
class GOLSAv2Continuous(PolicyBase):
    """
    GOLSAv2 with RL for continuous action space. Currently it's SAC.
    """
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        state_graphs.remove_loops_j(np.array([0, 1, 0, 2, 3, 4]))
        self.pos_diff_all = []
